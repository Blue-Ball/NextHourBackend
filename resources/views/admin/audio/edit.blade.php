...
@section('title',__('adminstaticwords.Edit')." "." - $audio->title")
<style type="text/css">
    body{
...
<div class="admin-form-main-block">
  <h4 class="admin-form-text"><a href="{{url('admin/audio')}}" data-toggle="tooltip" data-original-title="{{__('adminstaticwords.GoBack')}}" class="btn-floating"><i class="material-icons">reply</i></a> {{__('adminstaticwords.EditAudio')}}</h4>
  <div class="row">
    <div class="col-md-6">
     <div class="admin-form-block z-depth-1">
      ...
      <div id="movie_title" class="form-group{{ $errors->has('title') ? ' has-error' : '' }}">
        ...
        <i class="fa fa-info-circle" data-toggle="tooltip" data-placement="top" title="{{__('adminstaticwords.EnterAudioTitle')}}"></i>
         ...
         <div class="form-group{{ $errors->has('selecturl') ? ' has-error' : '' }}">
          ...
          <i class="fa fa-info-circle" data-toggle="tooltip" data-placement="top" title="{{__('adminstaticwords.PleaseSelectOneOfTheOptionsToAddAudio')}}"></i>
           <select class="form-control select2" id="selecturl" name="selecturl">
            ...
            @if($audio['audiourl']!='')
            <option value="audiourl" selected="">{{__('adminstaticwords.AudioURL')}}</option>
            @else
              <option value="audiourl">{{__('adminstaticwords.AudioURL')}}</option>
            @endif
            
           
             @if($audio['upload_audio']!='')
             <option value="upload_audio" selected="">{{__('adminstaticwords.UploadAudio')}}</option>
              @else
               <option value="upload_audio">{{__('adminstaticwords.UploadAudio')}}</option>
            @endif
      ...
      <div style="{{$audio['upload_audio']!='' ? '' : "display: none" }}"class="form-group{{ $errors->has('upload_audio') ? ' has-error' : '' }} input-file-block" id="uploadaudio" >
        ...
        <label for="upload_audio" class="btn btn-danger js-labelFile" data-toggle="tooltip" data-original-title="{{__('adminstaticwords.UploadAudio')}}">
          ...
          <span class="js-fileName">{{isset($audio->image) ? $audio->image :__('adminstaticwords.ChooseAFile')}}</span>
        </label>
