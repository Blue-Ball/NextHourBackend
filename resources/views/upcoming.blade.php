<div class="genre-prime-block genre-prime-block-one genre-paddin-top">
           <div class="container-fluid">
                ...
                <h5 class="section-heading">{{__('staticwords.UpcomingMovie')}} {{ $menu->name }}</h5>
                            ...
                            <div class="genre-prime-slide">
                              <div class="genre-slide-image home-prime-slider protip" data-pt-placement="outside" data-pt-title="#prime-mix-description-block{{$item->movie->id}}">
                                ...
                                @if($item->movie->is_custom_label == 1)
                                  @if(isset($item->movie->label_id))
                                    <span class="badge bg-success">{{$item->movie->label->name}}</span>
                                  @endif
                                @else
                                  ...
                                  @if(isset($item->movie->is_upcoming) && $item->movie->is_upcoming == 1)
                                    ...
                                    @else
                                      <span class="badge bg-success">{{__('Coming Soon')}}</span>
                                    @endif
                              ...
                              <div id="prime-mix-description-block{{$item->movie->id}}" class="prime-description-block">
                                  ...
                                  <ul class="description-list">
                                    <li>{{__('staticwords.tmdbrating')}} {{$item->movie->rating}}</li>
                                    <li>{{$item->movie->duration}} {{__('staticwords.mins')}}</li>
                                  ...
                                  <div class="main-des">
                                    ...
                                    @if($auth && $subscribed == 1)
                                      <a href="{{url('movie/detail',$item->movie->slug)}}">{{__('staticwords.readmore')}}</a>
                                    @else
                                       <a href="{{url('movie/guest/detail',$item->movie->slug)}}">{{__('staticwords.readmore')}}</a>
                                    @endif
                                  </div>
                                  ...
                                  <div class="des-btn-block">
                                    @if($auth && $subscribed==1)
                                      ...
                                      @if($item->movie->trailer_url != null || $item->movie->trailer_url != '')
                                         <a class="iframe btn btn-default" href="{{ route('watchTrailer',$item->movie->id) }}">{{__('staticwords.watchtrailer')}}</a>
                                      @endif
                                    @else
                                      @if($item->movie->trailer_url != null || $item->movie->trailer_url != '')
                                        <a class="iframe btn btn-default" href="{{ route('guestwatchtrailer',$item->movie->id) }}">{{__('staticwords.watchtrailer')}}</a>
                                      @endif
                                    @endif

                                    @if($catlog == 0 && $subscribed ==1)
                                      @if (isset($wishlist_check->added))
                                        <button onclick="addWish({{$item->movie->id}},'{{$item->movie->type}}')" class="addwishlistbtn{{$item->movie->id}}{{$item->type}} btn-default">{{$wishlist_check->added == 1 ? __('staticwords.removefromwatchlist') : __('staticwords.addtowatchlist')}}</button>
                                      @else
                                     
                                        <button onclick="addWish({{$item->movie->id}},'{{$item->movie->type}}')" class="addwishlistbtn{{$item->movie->id}}{{$item->type}} btn-default">{{__('staticwords.addtowatchlist')}}</button>
                                      @endif
                                    @elseif($catlog ==1 && $auth)
                                      ...
                                      @else
                                        ...
                                        <button onclick="addWish({{$item->movie->id}},'{{$item->movie->type}}')" class="addwishlistbtn{{$item->movie->id}}{{$item->movie->type}} btn-default">{{__('staticwords.addtowatchlist')}}</button>
                                      @endif
                                    @endif
                                ...
                                <div class="col-lg-2 col-md-3 col-xs-6 col-sm-4">
                                      ...
                                      @else
                                        ...
                                        @if(isset($item->movie->is_upcoming) && $item->movie->is_upcoming == 1)
                                          @if($item->upcoming_date != NULL)
                                            <span class="badge bg-danger">{{date('M jS Y',strtotime($item->upcoming_date))}}</span>
                                          @else
                                            <span class="badge bg-success">{{__('Coming Soon')}}</span>
                                          @endif
                                    ...
                                    <div id="prime-mix-description-block{{$item->movie->id}}" class="prime-description-block">
                                      ...
                                      <div class="des-btn-block">
                                        ...
                                        @if($catlog == 0 && $subscribed ==1)
                                          @if (isset($wishlist_check->added))
                                            <button onclick="addWish({{$item->movie->id}},'{{$item->movie->type}}')" class="addwishlistbtn{{$item->movie->id}}{{$item->movie->type}} btn-default">{{$wishlist_check->added == 1 ? __('staticwords.removefromwatchlist') : __('staticwords.addtowatchlist')}}</button>
                                          @else
