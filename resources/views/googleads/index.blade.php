...
@section('title',__('adminstaticwords.GoogleAdvertise'))
@section('stylesheet')
<style>
...
<div class="box-header">
  <h5>{{__('adminstaticwords.GoogleAdvertise')}}</h5>
</div>

	

        <a href="{{ route('google.ads.create') }}" class="btn btn-md btn-danger">+ {{__('adminstaticwords.CreateAD')}}</a>
      ...
      <div id="bulk_delete" class="delete-modal modal fade" role="dialog">
        ...
        <table id="full_detail_table" class="table table-hover">
            <thead>
                ...
                <th>#</th>
                <th>{{__('adminstaticwords.GoogleAdClient')}}</th>
                <th>{{__('adminstaticwords.GoogleAdSlot')}}</th>
                <th>{{__('adminstaticwords.Edit')}}</th>
            ...
            <tbody>
                ...
                <td>
                 ...
                 <td><a href="{{ route('google.ads.edit',$ad->id) }}" class="btn btn-sm btn-success">{{__('adminstaticwords.Edit')}}</a></td>
                 <td>
                     <form action="{{ route('google.ads.delete',$ad->id) }}" method="POST">
                        {{ csrf_field() }} 
                        {{ method_field('DELETE')}}
                        <input type="submit" value="{{__('adminstaticwords.Delete')}}" onclick="return confirm('Are you sure?')" class="btn btn-sm btn-danger"/>
