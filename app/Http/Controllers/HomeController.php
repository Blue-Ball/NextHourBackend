...
class HomeController extends Controller
{
    ...
    public function showall(Request $request, $menuid, $menuname)
    {
       ...
       if($request->title != NULL){
        ...
        $movies = $m->get()->toArray();

        $series = $tv->get()->toArray();
    ...
    public function guestindex(Request $request, $menu_slug)
    {
       ...
       }])->whereHas('menu_data.movie' , function($q){

            return $q->where('is_upcoming','!=','0');

       })->with(['menu_data.movie'=>function($q){
             return $q->where('is_upcoming','!=','0');
       }])->first();
       ...
       $audio_lang = $this->lang->select('id','language')->with(['movie','movie.video_link','seasons','seasons.episodes','seasons.episodes.video_link'])->get();
        ...
        if ($this->configs->prime_genre_slider == 1) {
            //Layout 1
            return view('home', compact('getallgenre', 'menu_data', 'recent_data', 'ad', 'age', 'genres', 'menuh', 'catlog', 'withlogin', 'menu', 'subscribed',  'audiolanguages', 'section6', 'blogs', 'liveevent', 'home_slides', 'age','audio','short_promo','audio_lang','upcomingitems','getallaudiolanguage','top_data'));
        } else {
            //Layout 2
            return view('home2', compact('getallgenre', 'menu_data', 'recent_data', 'ad', 'age', 'genres', 'menuh', 'catlog', 'withlogin', 'menu', 'subscribed',  'audiolanguages', 'section6', 'blogs', 'liveevent', 'home_slides', 'age','audio','short_promo','audio_lang','upcomingitems','getallaudiolanguage','top_data'));
        }
    ...
    public function search(Request $searchKey)
    {
       $age = null;
        ...
        $genre = $this->g->where('name', 'LIKE', "%$searchKey%")->first();
            ...
            foreach ($all_movies as $key => $item) {
                if ($item->genre_id != null && $item->genre_id != '') {
                    ...
                    for ($i = 0; $i < count($movie_genre_list); $i++) {
                        $check = $this->g->where('id', '=', trim($movie_genre_list[$i]))->get();
            ...
            foreach ($all_tvseries as $key => $item) {
                if ($item->genre_id != null && $item->genre_id != '') {
                    ...
                    for ($i = 0; $i < count($tv_genre_list); $i++) {
                        $check = $this->g->where('id', '=', trim($tv_genre_list[$i]))->get();
                        if (isset($check[0]) && $check[0]->name == $actor['name']) {
                            $filter_video->push($item);
                        }
                    }
        ...
        foreach ($all_movies as $item) {
            if ($item->a_language != null && $item->a_language != '') {
                ...
                for ($i = 0; $i < count($movie_lang_list); $i++) {
                    $check = Genre::find(trim($movie_lang_list[$i]));
        ...
        foreach ($all_seasons as $item) {
            if ($item->a_language != null && $item->a_language != '') {
                ...
                for ($i = 0; $i < count($season_lang_list); $i++) {
                    $check = Genre::find(trim($season_lang_list[$i]));
                    if (isset($check) && $check->id == $lang->id) {
                        $filter_video->push($item);
                    }
                }
